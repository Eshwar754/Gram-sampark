    ...
    accessRequestsUnsubscribe = onSnapshot(rq, (snapshot) => {
        ...
        snapshot.forEach(docSnap => {
            ...
            div.innerHTML = `
                ...
                <div class="col-actions">
                    <button class="icon-btn" onclick="approveAccess('${docSnap.id}', '${data.user_uid}', '${escapeHTML(data.user_email)}', '${escapeHTML(data.village)}')">Approve</button>
                    <button class="icon-btn delete" onclick="rejectAccess('${docSnap.id}')">Revoke</button>
                </div>
            `;
        ...
        div.innerHTML = `
            <div class="col-name">
                <div class="primary-text">
                    ${escapeHTML(p.name)}
                    <span class="badge ${p.source === 'Local' ? 'badge-warning' : 'badge-success'}" style="font-size: 0.7em; margin-left: 8px; padding: 2px 6px;">${p.source}</span>
                </div>
